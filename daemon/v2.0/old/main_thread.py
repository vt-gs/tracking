...
class Main_Thread(threading.Thread):
    ...
    def md01_connect_callback(self, con):
        #function called by MD01 thread when connection is established or lost to MD-01 controller
        self.md01_con = con
        if (self.user_con and self.md01_con): 
            #both user and MD-01 connected, transition to STANDBY
            self.set_state_standby()
        else:
            #MD-01 is NOT connected
            #User may or may not be connected
            self.set_state_idle()
    ...
    def run(self):
        ...
        while (not self._stop.isSet()): 
            if self.state == 'IDLE':
                ...
                if (self.md01_con == False) or (self.user_con == False): #user is not connected
                    self.set_state_idle()
